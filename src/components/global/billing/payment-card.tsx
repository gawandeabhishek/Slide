  ...
  return (
    ...
    >
      ...
      >
        ...
        {
            label === "PRO" ? (
                <span className="bg-gradient-to-r mt-2 text-3xl from-indigo-500 via-purple-500 font-bold to-pink-500 bg-clip-text text-transparent">Smart AI</span>
            ) : (
        ...
        {
            label == "PRO" ? (
                <p className="mb-2"><b className="text-xl">$99</b></p>
            ) : (
...

        {
            PLANS[label === 'PRO' ? 1 : 0].features.map((i) => (
                <p className="mt-2 text-muted-foreground flex gap-2" key={i}>
                    <CircleCheck className="text-indigo-500"/>{i}</p>
            ))
        }
